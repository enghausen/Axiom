...
try:
    xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=longLifeServiceManager")')
    ...
    xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=torrentCacheCleanup")')

    while not monitor.abortRequested():
        xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=runMaintenance")')
        if not g.wait_for_abort(15):  # Sleep to make sure tokens refreshed during maintenance
            xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=syncTraktActivities")')
        if not g.wait_for_abort(15):  # Sleep to make sure we don't possibly clobber settings
            xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=cleanOrphanedMetadata")')
        if not g.wait_for_abort(15):  # Sleep to make sure we don't possibly clobber settings
            xbmc.executebuiltin('RunPlugin("plugin://plugin.video.axiom/?action=updateLocalTimezone")')
